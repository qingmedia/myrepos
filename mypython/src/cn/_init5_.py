...
if os.path.exists("d://product/astringent")==False:
    os.makedirs("d://product/astringent")
errorTxt=open("d://product/astringent/error.txt" ,'a')
#读取html
def getHtml(url):
...
def parseData(urlList):
    urlW=open("d://product/astringent/url.txt" ,'a')
    for u in urlList:
...
#    (uid,categoryId,brand.decode('utf-8'),name.decode('utf-8'),imgUrl.decode('utf-8'),price.decode('utf-8'),description.decode('utf-8'),effect.decode('utf-8'),norms.decode('utf-8'),series.decode('utf-8'),storagedate) 
    sqlW=open("d://product/astringent/sql.txt" ,'a')
    print sql
    sqlW.write(sql)
    sqlW.write("\n")
    try:
