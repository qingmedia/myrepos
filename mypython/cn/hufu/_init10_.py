...
if os.path.exists("/usr/product/qujiaozhi")==False:
    os.makedirs("/usr/product/qujiaozhi")
errorTxt=open("/usr/product/qujiaozhi/error.txt" ,'a')
#读取html
def getHtml(url):
...
def insertData(name,brand,price,norms,effect,imgUrl,description,series):
    ...
    categoryId="402881AC4D411BBA014D411BBAF80012"
...
#    (uid,categoryId,brand.decode('utf-8'),name.decode('utf-8'),imgUrl.decode('utf-8'),price.decode('utf-8'),description.decode('utf-8'),effect.decode('utf-8'),norms.decode('utf-8'),series.decode('utf-8'),storagedate) 
    sqlW=open("/usr/product/qujiaozhi/sql.txt" ,'a')
    print sql
    sqlW.write(sql)
    sqlW.write("\n")
    try:
