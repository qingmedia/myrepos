...
if os.path.exists("/usr/product/jidiye")==False:
    os.makedirs("/usr/product/jidiye")
errorTxt=open("/usr/product/jidiye/error.txt" ,'a')
#读取html
def getHtml(url):
...
def parseData(urlList):
    urlW=open("/usr/product/jidiye/url.txt" ,'a')
    for u in urlList:
...
def insertData(name,brand,price,norms,effect,imgUrl,description,series):
    ...
    categoryId="402881AC4D411BBA014D411BBAF80006"
...
#    (uid,categoryId,brand.decode('utf-8'),name.decode('utf-8'),imgUrl.decode('utf-8'),price.decode('utf-8'),description.decode('utf-8'),effect.decode('utf-8'),norms.decode('utf-8'),series.decode('utf-8'),storagedate) 
    sqlW=open("/usr/product/jidiye/sql.txt" ,'a')
    print sql
    sqlW.write(sql)
    sqlW.write("\n")
    try:
    ...
    cursor.close()  
    db.close()    
#urlHtml=getHtml("http://cosme.pclady.com.cn/products_list/br0_bs0_bi1_sm95_ef0_pb0_pe0_or0.html")
for i in range(5 ,6):
    i=str(i)
    print i
    htmls="http://cosme.pclady.com.cn/products_list/br0_bs0_bi1_sm95_ef0_pb0_pe0_or0_p"+i+".html#productList"
