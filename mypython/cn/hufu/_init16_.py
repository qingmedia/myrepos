...
if os.path.exists("/usr/product/quheitou")==False:
    os.makedirs("/usr/product/quheitou")
errorTxt=open("/usr/product/quheitou/error.txt" ,'a')
#读取html
def getHtml(url):
...
def parseData(urlList):
    urlW=open("/usr/product/quheitou/url.txt" ,'a')
    for u in urlList:
...
def insertData(name,brand,price,norms,effect,imgUrl,description,series):
    ...
    categoryId="402881AC4D411BBA014D411BBAF80016"
...
#    (uid,categoryId,brand.decode('utf-8'),name.decode('utf-8'),imgUrl.decode('utf-8'),price.decode('utf-8'),description.decode('utf-8'),effect.decode('utf-8'),norms.decode('utf-8'),series.decode('utf-8'),storagedate) 
    sqlW=open("/usr/product/quheitou/sql.txt" ,'a')
    print sql
    sqlW.write(sql)
    sqlW.write("\n")
    try:
    ...
    cursor.close()  
    db.close()    
#urlHtml=getHtml("http://cosme.pclady.com.cn/products_list/br0_bs0_bi1_sm141_ef0_pb0_pe0_or0.html")
for i in range(3 ,4):
    i=str(i)
    print i
    htmls="http://cosme.pclady.com.cn/products_list/br0_bs0_bi1_sm141_ef0_pb0_pe0_or0_p"+i+".html#productList"
