...
#闁跨喐鏋婚幏宄板絿html
if os.path.exists("/usr/product/mianmo")==False:
    os.makedirs("/usr/product/mianmo")
errorTxt=open("/usr/product/mianmo/error.txt" ,'a')
def getHtml(url):
#    h=urllib2.urlopen(url, '', 3000).read()
    opener = urllib2.build_opener(urllib2.HTTPCookieProcessor())
    h=opener.open(url).read()
    return h
#闁跨喐鏋婚幏鐑芥晸閺傘倖瀚箈ml 闁跨喐鏋婚幏宄板絿闁跨喐鏋婚幏鐤渽闁跨喐鏋婚幏鐑芥晸閺傘倖瀚归柨鐕傛嫹
...
def insertData(name,brand,price,norms,effect,imgUrl,description,series):
    ...
    categoryId="402881AC4D411BBA014D411BBAF80009"
...
#    (uid,categoryId,brand.decode('utf-8'),name.decode('utf-8'),imgUrl.decode('utf-8'),price.decode('utf-8'),description.decode('utf-8'),effect.decode('utf-8'),norms.decode('utf-8'),series.decode('utf-8'),storagedate) 
    sqlW=open("/usr/product/mianmo/sql.txt" ,'a')
    print sql
    sqlW.write(sql)
    sqlW.write("\n")
    try:
    ...
    cursor.close()  
    db.close()    
#urlHtml=getHtml("http://cosme.pclady.com.cn/products_list/br0_bs0_bi1_sm6_ef0_pb0_pe0_or0.html#productList")
for i in range(333,334):
    if i<10:
        ...
        i=str(i)
    print i
    htmls="http://cosme.pclady.com.cn/products_list/br0_bs0_bi1_sm6_ef0_pb0_pe0_or0_p"+i+".html#productList"
