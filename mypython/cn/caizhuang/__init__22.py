...
#读取html
if os.path.exists("/usr/caizhuang/dingzhuangpin")==False:
    os.makedirs("/usr/caizhuang/dingzhuangpin")
errorTxt=open("/usr/caizhuang/dingzhuangpin/error.txt" ,'a')
def getHtml(url):
...
def parseData(urlList):
    urlW=open("/usr/caizhuang/dingzhuangpin/url.txt" ,'a')
    for u in urlList:
...
def insertData(name,brand,price,norms,effect,imgUrl,description,series):
    ...
    categoryId="402881AC4D4188E9014D4188E99D0002"
...
#    (uid,categoryId,brand.decode('utf-8'),name.decode('utf-8'),imgUrl.decode('utf-8'),price.decode('utf-8'),description.decode('utf-8'),effect.decode('utf-8'),norms.decode('utf-8'),series.decode('utf-8'),storagedate) 
    sqlW=open("/usr/caizhuang/dingzhuangpin/sql.txt" ,'a')
    print sql
    sqlW.write(sql)
    sqlW.write("\n")
    try:
    ...
    cursor.close()  
    db.close()    
urlHtml=getHtml("http://cosme.pclady.com.cn/products_list/br0_bs0_bi2_sm199_ef0_pb0_pe0_or0.html")
...
for i in range(2,4):
    if i<10:
        ...
        i=str(i)
    print i
    htmls="http://cosme.pclady.com.cn/products_list/br0_bs0_bi2_sm199_ef0_pb0_pe0_or0_p"+i+".html#productList"
